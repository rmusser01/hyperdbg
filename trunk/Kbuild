...
DEFINE += -DHVM_ARCH_BITS=32 -DENABLE_HYPERDBG \
	  -DGUEST_LINUX -DVIDEO_DEFAULT_RESOLUTION_X=1024 -DVIDEO_DEFAULT_RESOLUTION_Y=768 -DVIDEO_ADDRESS_MANUAL -DHYPERDBG_VERSION=$(shell date +%Y%m%d)
...
core-objs:= $(core-src)/pill_linux.o $(core-src)/pill_common.o \
	    ...
	    $(core-src)/process.o $(core-src)/network.o $(core-src)/vt.o 
...
hyperdbg-objs:= $(hdbg-src)/gui.o $(hdbg-src)/font_256.o  $(hdbg-src)/hyperdbg_cmd.o $(hdbg-src)/hyperdbg_guest.o \
	        $(hdbg-src)/hyperdbg_host.o $(hdbg-src)/keyboard.o $(hdbg-src)/pager.o $(hdbg-src)/pci.o \
