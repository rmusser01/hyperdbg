...
gen_operand(struct ud* u, struct ud_operand* op)
{
  switch(op->type) {
	...
	case UD_OP_MEM:
		...
		if (op->offset == 8) {
			if (op->lval.sbyte < 0)
				mkasm(u, "-0x%hx", (-op->lval.sbyte) & 0xff);
			else	mkasm(u, "0x%hx", op->lval.sbyte);
		} 
		else if (op->offset == 16) 
			mkasm(u, "0x%hx", op->lval.uword);
		else if (op->offset == 32) 
			mkasm(u, "0x%hx", op->lval.udword);
	...
	case UD_OP_IMM:
		switch (op->size) {
			case  8: mkasm(u, "$0x%hx", op->lval.ubyte);    break;
			case 16: mkasm(u, "$0x%hx", op->lval.uword);    break;
			case 32: mkasm(u, "$0x%hx", op->lval.udword);  break;
	...
	case UD_OP_PTR:
		switch (op->size) {
			case 32:
				mkasm(u, "$0x%hx, $0x%hx", op->lval.ptr.seg, 
...
ud_translate_att(struct ud *u)
{
  ...
  switch (u->mnemonic) {
	case UD_Iretf: 
		...
		break;
	case UD_Idb:
		mkasm(u, ".byte 0x%hx", u->operand[0].lval.ubyte);
		return;
	case UD_Ijmp:
	case UD_Icall:
